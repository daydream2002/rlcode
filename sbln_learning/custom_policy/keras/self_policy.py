...
class selfPolicy(FeedForwardPolicy):
    '''
        自定义AC策略
        className:selfPolicy
        fileName:self_policy.py
    '''
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=None,
        ...
        with tf.variable_scope("model", reuse=reuse):
            if feature_extraction == "cnn":
                ...
                latent_policy=Dense(64, activation='relu', name="latent_policy")(x)
                pi_latent = Dense(64, activation='relu', name="pi_latent")(latent_policy)


                latent_value = Dense(64, activation='relu', name="latent_value")(x)
                vf_latent = Dense(64, activation='relu', name="vf_latent")(latent_value)
                #x = Dense(self.output_features, activation='softmax')(x)

                #conv1=tf.layers.conv2d(inputs=self.processed_obs,filters=32,kernel_size=[3,3],strides=1,padding='same',activation=tf.nn.relu)
                ...
                latent_policy = tf.nn.relu(linear(latent, "pi_fc1", 64, init_scale=np.sqrt(2)))
                pi_latent = tf.nn.relu(linear(latent_policy, "pi_fc2", 64, init_scale=np.sqrt(2)))

                latent_value = tf.nn.relu(linear(latent, "vf_fc3", 64, init_scale=np.sqrt(2)))
                vf_latent = tf.nn.relu(linear(latent_value, "vf_fc4", 64, init_scale=np.sqrt(2)))
                '''
