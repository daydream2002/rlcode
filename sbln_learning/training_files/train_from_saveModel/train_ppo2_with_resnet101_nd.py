...
# @Time    : 2023/3/3
# @Author  : xiu
# @File    : train_ppo2_with_resnet101.py
# @Description:ResNeti101测了从预训练模型继续训练
import sys
sys.path.append('../../../')
sys.path.append("../")
import os
...
ENT_COEF = 0.001  # 策略熵的折扣系数，默认0.01  微调
VF_COEF = 0.5  # loss的熵函数系数 默认0.5  不变
TENSORBOARD_LOG = "./tensorboardLog"  # tensorboard的存放路径
FULL_LOG = False  # 是否记录所有数据记录

DynEnt = "_dynEnt_0.04"  # "_dynEnt_0.5"  # 是否动态调节策略熵  数字必须放到后面，并且用_隔开
DynAppkl = "dynAppkl_0.5"  # 是否动态调整kl散度
...
TENSORBOARD_LOG_NAME = "PPO2_fea1274_"+DATACLIP+"_"+"lr_start"+str(INIT_LR)+"end"+str(END_LR)+"_"+POLICY+"_gm"+str(GAMMA)+"_clip"+str(INIT_CLIPRANGE)+"_entCoef"+\
                       str(ENT_COEF)+"_sqrtR_3pv5_mid"+MidReward + DynEnt+DynAppkl
...
SAVEMODEL_REWARD = -0.2  # 从多少reward开始保存模型


MODEL_PATH = "./save_model/ppo2_policyResnet101_tf_Nfea1274_updateStep2048_lr0.00028860349439999996_" \
             "gamma0.925_entCoef-0.03278114010654392_vfCoef0.5_reward0.9359512007854218_GangIsFalse_MidReward0.01_" \
             "illegaA-3_abort_reward-1_nsteps_4222976" # 从哪个模型开始恢复训练
# 从哪个模型开始恢复训练
...
INFO_ON_LOG = {"lr": INIT_LR, "lr_info": "LinearSchedule; end_lr:"+str(END_LR),"n_fea": 1274,
               ...
               "GameInfo": {"opponent": ["v5", "v5", "v5"], "midReward": MidReward, "sqrtReward": True,
