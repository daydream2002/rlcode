...
# @Description:使用ResNet101网络从0开始训练出牌模型
import sys
...
config.gpu_options.per_process_gpu_memory_fraction = 0.3  # 设置显存占用率大小
...
DynEnt = ""  # _dynEnt_0.5"  # "_dynEnt_0.5"  # 是否动态调节策略熵  数字必须放到后面，并且用_隔开
...
TENSORBOARD_LOG_NAME = "PPO2_fea1274_" + DATACLIP + "lr_start" + str(INIT_LR) + "end" + str(
    END_LR) + "_" + POLICY + "_gm" + str(GAMMA) + "_clip" + str(INIT_CLIPRANGE) + "_entCoef" + \
                       str(ENT_COEF) + "_sqrtR_3p_mid" + str(MidReward) + DynEnt
...
INFO_ON_LOG = {"lr": INIT_LR, "lr_info": "LinearSchedule; end_lr:" + str(END_LR), "n_fea": 1274,
               ...
               "policy": POLICY, "gpu": GPU, "dynEnt": DynEnt,
...
model.save(
    ...
    "_vfCoef" + str(VF_COEF) + "_end_steps_" + str(TOTAL_TIMESTEPS) + "_1pzhiyi_sqrtR_3p_mid" + str(MidReward) + DynEnt,
    True)
