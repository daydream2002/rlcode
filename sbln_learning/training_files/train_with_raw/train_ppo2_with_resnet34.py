...
config.gpu_options.per_process_gpu_memory_fraction = 0.2  # 设置显存占用率大小
...
ENT_COEF = 0.01  # 策略熵的折扣系数，默认0.01  微调
...
LEARNING_RATE = LinearSchedule(TOTAL_TIMESTEPS, initial_p=INIT_LR,
                               final_p=INIT_LR / 10).value  # (float or callable) The learning rate, it can be a function

TENSORBOARD_LOG_NAME = POLICY + "_PPO2_fea488" + "lr_" + str(INIT_LR) + "_Res18_gm" + str(GAMMA) + "_clip" + str(
    INIT_CLIPRANGE) + "_entCoef" + \
                       str(ENT_COEF) + "_sqrtR"
...
INFO_ON_LOG = {"lr": INIT_LR, "lr_info": "LinearSchedule; end_lr:" + str(INIT_LR / 10), "n_fea": 488,
